...
$("#myform1").on("click", function (e) {
  ...
  if (!AdaniInvoiceNumber) {
    $("#number").after(
      '<span class="error" id="errorvalidation" style="color: red;">This field is required</span>'
    );
  ...
  if (!/^[0-9]+$/.test(AdaniInvoiceNumber)) {
    $("#number").after(
      '<span class="error" id="errorvalidation" style="color: red;">Please enter valid Invoice number </span>'
    );
    $("#errorvalidation").show();
    return false;
  }
          ...
          $("#myTable")
            ...
            .append(
              "<tr><td>" +
                ...
                "</td><td>" +
                item.Serial_No +
                '</td><td><input type="checkbox" id="cb' +
                item.Serial_No +
                '" name="serialData" class="dtCheckbox" onclick="CountSelectedCheckbox()" value=' +
                item.Serial_No +
                " data-PalletID=" +
                item.Pallet_ID +
...
$(document).ready(function () {
  $("#downloadFileButton").click(function () {
    var getSelectedCheckboxValue = $.map(
      $("input:checkbox[name=serialData]:checked"),
      function (n, i) {
        return n.value;
      }
    ).join(",");
    if (getSelectedCheckboxValue !== "") {
      ...
      for (var i = 0; i < getCheckboxIDs.length; i++) {
        ...
        warrantydate =
          $(cbID).attr("data-WarrantyEndDate").substr(6, 2) +
          "-" +
          $(cbID).attr("data-WarrantyEndDate").substr(4, 2) +
          "-" +
          $(cbID).attr("data-WarrantyEndDate").slice(0, 4);
        cerinput.push({
          palletId: $(cbID).attr("data-PalletID"),
          moduleSerialNumber: $(cbID).attr("value"),
          performanceWarrantyTill: warrantydate,
        });
        var formdata = new FormData();
        formdata.append("Invoice_Number", $(cbID).attr("value"));
        var UserName = $("#consumerName").val();
        UpdateDownloadHistory(
          $(cbID).attr("data-PalletID"),
          $(cbID).attr("value"),
          warrantydate,
          UserName
        );
      ...
      window.open("show-certificate.html", "_blank");
    }
  });
});
...
$("#addSerialRow").click(function (e) {
  ...
  var childCount =
    $(".remove").last().attr("id") == undefined
      ? 1
      : parseInt($(".remove").last().attr("id")) + 1;
...
$("#generateOTPbtn").click(function (e) {
  ...
  if (!validateEmail(consumerEmail)) {
    $("#consumerEmail").after(
      '<p class="text-danger">Can you please enter correct EmailID ?</p>'
    );
    $("#consumerEmail").focus();
    $("#consumerEmail").addClass("error");
    return false;
...
function Isvalidate() {
  ...
  if (!consumerAddress) {
    $("#consumerAddress").after(
      '<p class="text-danger">Address required ?</p>'
    );
