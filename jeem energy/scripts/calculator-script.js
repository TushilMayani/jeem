...
$(document).ready(function () {
  ...
  $("#states").change(function () {
    ...
    //$('.states').hide();
    $("#state_info").html(
      "<p> Average solar irradiation in <span><b>" +
        states_data[state_id]["name"] +
        " </span></b>is <span><b>" +
        states_data[state_id]["avg_solar_irradiation"]["value"] +
        " W / sq.m</span></b><b/></p><p>1kWp solar rooftop plant will generate " +
        states_data[state_id]["elec_generated_per_kwp_rftp_plant"]["value"] +
        " kWh of electricity per day (considering 5.5 sunshine hours)</p>"
    );
    $("#state_info").show();
...
$(document).ready(function () {
  ...
  $("input[name=debt]").change(function () {
    ...
    var down_payment =
      ($("input[name=solar_plant_cost]").val() *
        $("input[name=equity]").val()) /
      100;
    $("input[name=dwn_pymt]").val(down_payment);

    var loan_amount =
      ($("input[name=solar_plant_cost]").val() * $("input[name=debt]").val()) /
      100;
    $("input[name=loan_amt]").val(loan_amount);
  });
  ...
  $("input[name=dwn_pymt]").change(function () {
    if (
      ...
      $("input[name=loan_amt]").val(
        $("input[name=solar_plant_cost]").val() -
          $("input[name=dwn_pymt]").val()
      );
    else {
      $("input[name=dwn_pymt]").val($("input[name=solar_plant_cost]").val());
    ...
    $("input[name=equity]").val(
      ($("input[name=dwn_pymt]").val() * 100) /
        $("input[name=solar_plant_cost]").val()
    );
    $("input[name=debt]").val(
      ($("input[name=loan_amt]").val() * 100) /
        $("input[name=solar_plant_cost]").val()
    );
  });
...
$(document).ready(function () {
  $("#form_submit").click(function (event) {
    ...
    if (
      $("#roof_top_area").val() == "" &&
      $("#install_capacity").val() == "" &&
      $("#budget").val() == ""
    ) {
      var dialog = new Messi(
        "Please enter one of the Total roof area, Capacity you want to install or Budget."
      );
      return;
    }
    ...
    if ($("#roof_top_area").val() != "") {
      if (!$.isNumeric($("#roof_top_area").val())) {
        var dialog = new Messi(
          "Roof top area should be numeric be numeric only"
        );
        return;
      }
    }
    ...
    if ($("#roof_top_area_percentage").val() != "") {
      if (!$.isNumeric($("#roof_top_area_percentage").val())) {
        var dialog = new Messi(
          "Roof top area percentage should be numeric be numeric only"
        );
        return;
      }
    }
    ...
    if ($("#customerType").val() == "" || $("#electricityCost").val() == "") {
      var dialog = new Messi(
        "Please select customer type or give electricity cost."
      );
      return;
    ...
    var capacity = 0;
    if ($("input[name=estimation_base]:checked").val() == "roof_top") {
      //user has given roof top area

      //clear other values if filled

      if ($("input[name=roof_top_area_unit]:checked").val() == "sqm") {
        capacity =
          ($("#roof_top_area").val() * $("#roof_top_area_percentage").val()) /
          100 /
          constant_for_sqm;
        //alert(capacity);
      } else {
    ...
    } else if ($("input[name=estimation_base]:checked").val() == "capacity") {
      // user has given capacity
      capacity = $("#install_capacity").val();
    } else {
      // user had given budget
    ...
    $("#cost_with_subsidy").text(
      Math.round(per_kw_plant_cost * capacity - actual_subsidy)
    );

    $("#plant_size").text(Math.round(capacity)); // plant size is capacity

    var annual_generate_electricity = Math.round(
      capacity * solar_radiance * min_efficiency_expected
    ); // supposing it is yearly

    //alert('total elect' + annual_generate_electricity);

    $("#annual_generated_elec").text(annual_generate_electricity);
    $("#life_time_generated_elec").text(
      annual_generate_electricity * life_time
    );

    //without increasing tarrif
    var x = 1;
    var total_saved_electricity_per_year = Math.round(
      annual_generate_electricity * $("#electricityCost").val() * x
    );
    //alert(total_saved_electricity_per_year);
    $("#saving_without_increasing_tarrif_annualy").text(
      total_saved_electricity_per_year.toString()
    );
    $("#saving_without_increasing_tarrif_monthly").text(
      Math.round(total_saved_electricity_per_year / 12)
    );
    $("#saving_without_increasing_tarrif_life_time").text(
      Math.round(total_saved_electricity_per_year * life_time)
    );
    ...
    var total_saved_electricity_per_year = Math.round(
      annual_generate_electricity * $("#electricityCost").val() * x
    ); // because only on year is there.
    $("#saving_with_increasing_tarrif_annually").text(
      total_saved_electricity_per_year
    );

    var total_saved_electricity_life_time = Math.round(
      annual_generate_electricity *
        $("#electricityCost").val() *
        Math.pow(x, life_time)
    );
    $("#saving_with_increasing_tarrif_life_time").text(
      total_saved_electricity_life_time
    );
    ...
    //co2 mitigation
    var co2_mitigated = Math.round(
      (annual_generate_electricity * life_time * co2_mitigation_constant) / 1000
    );
    ...
    //pay back
    $("#self_without_subsidy_present").text(
      Math.round($("#cost_without_subsidy").text() / $("#emi").text())
    );
    $("#self_with_subsidy_present").text(
      Math.round($("#cost_with_subsidy").text() / $("#emi").text())
    );
