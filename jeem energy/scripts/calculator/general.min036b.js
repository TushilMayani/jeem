function calculate() {
  if (
    (validateForm(), "" == $("#state").val() || "" == $("#customer_type").val())
  )
    return !1;
  $("#emi_data").hide();
  ...
  if (t) {
    if (
      ...
      (_ = calculate_plant_size_area()) > 500)
    )
      return (
        (message =
    ...
    if (_ > 500)
      return (
        (message =
          "Maximum limit of capacity is 500kW. Please try less than or equal to 500kW"),
        new Messi(message, {
          title: "Error",
          titleClass: "anim error",
          modal: !0,
    ...
    if (
      ...
      (_ = calculate_plant_size_budget(budget, r, i)) > 500)
    )
      return (
        (message =
          "As per budget ,capacity found is beyond 500kW. Maximum limit of capacity is 500kW"),
        new Messi(message, {
          title: "Error",
          titleClass: "anim error",
          modal: !0,
    ...
    budget < c
      ? ((output_without_subsidy = c),
        (budget_additionally =
          " plus Rs " + (Number(c) - Number(budget)) + " more"))
      : (output_without_subsidy = $("#budget_txt").val());
  }
  ...
  (subsidy2 = 1 - d / 100),
    (output_subsidy = output_without_subsidy * subsidy2),
    (output_subsidy = Number(output_subsidy).toFixed(0)),
    (irradiation = get_irradiation(a)),
    ...
    (value_a = 0.82 * output_electricity_lifetime),
    (value_b = 625),
    (co2 = value_a / 1e3),
    ...
    $("#output_size").html(_),
    $("#bench_cost").html(c),
    $("#output_without_subsidy").html(output_without_subsidy),
    $("#output_subsidy_heading").html(d),
    $("#output_subsidy").html(output_subsidy),
    ...
    $("#capital_cost_txt").val(output_without_subsidy),
    $("#subsidy_txt").val(d),
    $("#equity_ratio_txt").val(d),
    (debt_ratio = 100 - d),
    $("#debt_ratio_txt").val(debt_ratio),
    (down_payment = (d / 100) * output_without_subsidy),
    (down_payment = Math.round(down_payment)),
    (loan_payment = output_without_subsidy - down_payment),
    $("#down_payment_txt").val(down_payment),
    $("#loan_payment_txt").val(loan_payment),
  ...
  console.log(output_electricity_annual);
  console.log(output_electricity_lifetime);
  console.log(output_saving_monthly);
  console.log(output_saving_annually);
  console.log(output_saving_lifetime);
}

function emi_calculate() {
  $(".emi_output").html(""),
    "" != $("#loan_inst_rate_txt").val() &&
      "" != $("#loan_period_txt").val() &&
      ((down_payment = $("#down_payment_txt").val()),
      (loan_payment = $("#loan_payment_txt").val()),
      (inst_rate = $("#loan_inst_rate_txt").val()),
      (loan_period = $("#loan_period_txt").val()),
      (p = loan_payment),
      (r = inst_rate / 100),
      (r /= 12),
      (tenure = 12 * loan_period),
      (r_pow = Math.pow(1 + r, tenure)),
      (num_emi = r * r_pow),
      (den_emi = r_pow - 1),
      (emi = p * (num_emi / den_emi)),
      (emi = Number(emi).toFixed(0)),
      $("#output_emi_amount").html(loan_payment),
      $("#output_loan_interest").html(inst_rate),
      $("#output_loan_period").html(loan_period),
      $("#calculated_emi").html(emi),
      $("#emi_data").show());
}
...
function calculate_plant_size_area() {
  return (
    ...
    (capacity = (roof_area * roof_area_percentage) / 1e3),
...
function get_benchmark(t, e, a) {
  t = parseInt(t);
  for (benchmark = 0, i = 0; i < data.length; i++)
    data[i].category == e &&
      data[i].customer == a &&
      t >= data[i].rangeFrom &&
      t <= data[i].rangeTo &&
      ((cost = data[i].cost), (benchmark = 1e3 * cost));
  if (0 == benchmark)
    for (i = 0; i < data.length; i++)
      data[i].category == e &&
        t >= data[i].rangeFrom &&
...
function get_state_category(t) {
  for (i = 0; i < data_state_catg.length; i++)
    data_state_catg[i].state == t && (category = data_state_catg[i].category);
  return category;
}

function get_irradiation(t) {
  for (i = 0; i < data_irradiation.length; i++)
    data_irradiation[i].state == t &&
      (irradiation = data_irradiation[i].irradiation);
  return irradiation;
}

function get_irradiation_ele(t) {
  for (i = 0; i < data_irradiation_ele.length; i++)
    data_irradiation_ele[i].state == t &&
      (irradiation_ele = data_irradiation_ele[i].irradiation_ele);
  return irradiation_ele;
}

function check_fill(t) {
  $(".chk-grp").val(""),
    $(".chk-grp").attr("disabled", !0),
    $(".chk-grp").hide(),
    $(".txt_option").hide(),
    "checkbox_fill_one" == t.id && t.checked
      ? ($(".txt_option1").show(),
        $(".chk-grp1").show(),
        $(".chk-grp1").attr("disabled", !1),
        $("#checkbox_fill_two").attr("checked", !1),
        $("#checkbox_fill_three").attr("checked", !1))
      : "checkbox_fill_two" == t.id && t.checked
      ? ($(".txt_option2").show(),
        $(".chk-grp2").show(),
        $(".chk-grp2").attr("disabled", !1),
        $("#checkbox_fill_one").attr("checked", !1),
        $("#checkbox_fill_three").attr("checked", !1),
        $("#radio_gp").prop("checked", !1))
      : "checkbox_fill_three" == t.id &&
        t.checked &&
        ($(".txt_option3").show(),
        $(".chk-grp3").show(),
        $(".chk-grp3").attr("disabled", !1),
        $("#checkbox_fill_one").attr("checked", !1),
        $("#checkbox_fill_two").attr("checked", !1),
        $("#radio_gp").prop("checked", !1));
}
...
function check_fill_one() {
  $("#checkbox_fill_one").is(":checked")
    ? ($("#roof_area_txt").val(""),
      $("#roof_area_percentage_txt").val(""),
      $("#roof_area_txt").attr("disabled", !1),
      $("#roof_area_percentage_txt").attr("disabled", !1),
...
function check_fill_two() {
  $("#checkbox_fill_two").is(":checked")
    ? ($("#roof_area_txt").attr("disabled", !0),
      $("#roof_area_percentage_txt").attr("disabled", !0),
      $("#capacity_txt").val(""),
      $("#capacity_txt").attr("disabled", !1),
      $("#budget_txt").attr("disabled", !0),
      $("#checkbox_fill_one").attr("checked", !1),
      $("#checkbox_fill_three").attr("checked", !1),
      $("#radio_gp").prop("checked", !1))
...
function check_fill_three() {
  $("#checkbox_fill_three").is(":checked")
    ? ($("#roof_area_txt").attr("disabled", !0),
      $("#roof_area_percentage_txt").attr("disabled", !0),
      $("#capacity_txt").attr("disabled", !0),
      $("#budget_txt").val(""),
      $("#budget_txt").attr("disabled", !1),
      $("#checkbox_fill_one").attr("checked", !1),
      $("#checkbox_fill_two").attr("checked", !1),
      $("#radio_gp").prop("checked", !1))
    : ($("#roof_area_txt").val(""),
...
function calculate_ratio() {
  (capital_cost = $("#capital_cost_txt").val()),
    (debt_ratio = $("#debt_ratio_txt").val()),
    (equity_ratio = $("#equity_ratio_txt").val()),
    (loan_payment = (debt_ratio / 100) * capital_cost),
    (loan_payment = Math.round(loan_payment)),
    (down_payment = capital_cost - loan_payment),
    $("#down_payment_txt").val(down_payment),
    $("#loan_payment_txt").val(loan_payment);
}
  ...
  (capital_cost = $("#capital_cost_txt").val()),
    ...
    0 != debt_ratio
      ? ((equity_ratio = 100 - debt_ratio),
        $("#equity_ratio_txt").val(equity_ratio),
        (loan_payment = (debt_ratio / 100) * capital_cost),
        (loan_payment = Math.round(loan_payment)),
        $("#loan_payment_txt").val(loan_payment),
        (down_payment = capital_cost - loan_payment),
      ...
      : ((equity_ratio = 100 - debt_ratio),
        $("#equity_ratio_txt").val(equity_ratio),
        $("#loan_payment_txt").val("0"),
        $("#down_payment_txt").val(capital_cost));
}
  ...
  (capital_cost = $("#capital_cost_txt").val()),
    ...
    0 != equity_ratio
      ? ((debt_ratio = 100 - equity_ratio),
        $("#debt_ratio_txt").val(debt_ratio),
        (down_payment = (equity_ratio / 100) * capital_cost),
        (down_payment = Math.round(down_payment)),
        $("#down_payment_txt").val(down_payment),
        (loan_payment = capital_cost - down_payment),
        (loan_payment = Math.round(loan_payment)),
        $("#loan_payment_txt").val(loan_payment))
      : ((debt_ratio = 100 - equity_ratio),
        $("#debt_ratio_txt").val(debt_ratio),
        $("#down_payment_txt").val("0"),
        $("#loan_payment_txt").val(capital_cost));
}
...
function calculate_down_payment() {
  if (
    ((capital_cost = $("#capital_cost_txt").val()),
    (down_payment = $("#down_payment_txt").val()),
    down_payment > capital_cost)
  )
    return (
      (message = "Down payment can not be greater than Input Capital Cost!"),
      new Messi(message, {
        title: "Error",
        titleClass: "anim error",
        modal: !0,
  ...
  (loan_payment = capital_cost - down_payment),
    $("#loan_payment_txt").val(loan_payment),
    (equity_ratio = (down_payment / capital_cost) * 100),
    (equity_ratio = Math.round(equity_ratio)),
    $("#equity_ratio_txt").val(equity_ratio),
    (debt_ratio = 100 - equity_ratio),
    $("#debt_ratio_txt").val(debt_ratio);
}
...
function calculate_loan_payment() {
  if (
    ((capital_cost = $("#capital_cost_txt").val()),
    (loan_payment = $("#loan_payment_txt").val()),
    loan_payment > capital_cost)
  )
    return (
      (message = "Loan payment can not be greater than Input Capital Cost!"),
      new Messi(message, {
        title: "Error",
        titleClass: "anim error",
        modal: !0,
  ...
  (down_payment = capital_cost - loan_payment),
    $("#down_payment_txt").val(down_payment),
    (debt_ratio = (loan_payment / capital_cost) * 100),
    (debt_ratio = Math.round(debt_ratio)),
    $("#debt_ratio_txt").val(debt_ratio),
    (equity_ratio = 100 - debt_ratio),
    $("#equity_ratio_txt").val(equity_ratio);
}
...
function validateForm() {
  ...
  if (t) {
    if ("" == document.frm.roof_area_txt.value)
      return (
        (message = "Please Enter Total Roof Top Area!"),
        new Messi(message, {
          title: "Error",
          titleClass: "anim error",
          modal: !0,
    ...
    if ("" == o)
      return (
        (message = "Please Enter Percentage of Roof Top Area !"),
        new Messi(message, {
          title: "Error",
          titleClass: "anim error",
          modal: !0,
    ...
    if ("" != o && !percent(o))
      return (
        (message = "Invalid Percentage of Roof Top Area"),
        new Messi(message, {
          title: "Error",
          titleClass: "anim error",
          modal: !0,
    ...
    if (!a)
      return (
        (message = "Please choose any one of the 3 Plant Size criteria"),
        new Messi(message, {
          title: "Error",
          titleClass: "anim error",
          modal: !0,
  ...
  return "" == r
    ? ((message = "Please Enter average Electricity Cost"),
      new Messi(message, {
        title: "Error",
        titleClass: "anim error",
        modal: !0,
    ...
    : "" == r || Number(r)
    ? "" != r && Number(r) > 100
      ? ((message = "Invalid average Electricity Cost"),
        new Messi(message, {
          title: "Error",
          titleClass: "anim error",
          modal: !0,
        ...
        !1)
      : !("" != r && Number(r) > 20) ||
        ((message = "Very high average Electricity Cost"),
        new Messi(message, {
          title: "Error",
          titleClass: "anim error",
          modal: !0,
...
function number(t) {
  ...
  return (
    8 == e || 0 == e || (String.fromCharCode(e).toLowerCase(), e > 47 && e < 58)
  );
}
$(document).ready(function () {
  (baseurl = $("#url").val()),
    ...
    $(".chk-grp").attr("disabled", !0),
    $(".chk-grp").hide(),
    $("#capital_cost_txt").prop("disabled", !0),
    $("#subsidy_txt").prop("disabled", !0),
    $("#emi_data").hide();
});
